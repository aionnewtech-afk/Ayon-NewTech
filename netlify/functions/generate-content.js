...
Use a ferramenta "gerar_post" para entregar o resultado final.`;
  ...
  try {
        ...
        tools: [{
          ...
          input_schema: {
            ...
            properties: {
              ...
              highlight: { type: 'string', description: 'Um resumo bem curto (máximo 15 palavras), tipo manchete/headline de impacto, capturando a essência do post. Vai ser usado como destaque grande numa imagem — não é o post inteiro, é só a frase mais forte.' }
            },
            required: ['linkedin', 'instagram', 'highlight']
          }
        }],
    ...
    if (!toolUse || !toolUse.input || !toolUse.input.linkedin || !toolUse.input.instagram || !toolUse.input.highlight) {
    ...
    return {
      ...
      body: JSON.stringify({ linkedin: toolUse.input.linkedin, instagram: toolUse.input.instagram, highlight: toolUse.input.highlight })
    };
  } catch (err) {
